...
/**
 * @deprecated - will be absolutely reworked, such as
 * all the post system
 */
...
public class PostService {
    ...
    public void save(@NotNull Post post) {
        ...
        if (post.getFulltextTitle().isBlank() || post.getFulltextTitle().matches("[^aA-zZ аА-яЯ0-9]")) {
            String fulltext = makeFulltext(post.getTitle());
            post.setFulltextTitle(fulltext.isBlank() ? post.getTitle() : fulltext);
        }
        if (post.getFulltextContent().isBlank() || post.getFulltextContent().matches("[^aA-zZ аА-яЯ0-9]")) {
            String fulltext = makeFulltext(post.getContent());
            post.setFulltextContent(fulltext.isBlank() ? post.getContent() : fulltext);
        }
